...
(def select-queries
  {:find-by-rid "select from Has where @rid = :rid"
   :find-by-from-and-to "select from Has
                         where out = :from and in = :to"})

(def insert-queries
  {:create "create edge Has from :from to :to
...
(defn create
  "Create a Has edge from two vertexes if not existing.
